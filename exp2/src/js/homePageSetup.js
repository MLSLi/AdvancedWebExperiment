...
export default function useHomePage() {
  ...
  const submitRegister = async () => {
    ...
    try {
      ...
      const isDuplicate = existingUsers.some((user) => user.username === username.value)
  ...
  const submitLogin = async () => {
    ...
    try {
      const { data: existingUsers } = await axios.get('http://localhost:3000/users')
      const inputName = username.value.trim().toLowerCase()
      const matchedUser = existingUsers.find(
        (user) => user.username?.trim().toLowerCase() === inputName,
      )
